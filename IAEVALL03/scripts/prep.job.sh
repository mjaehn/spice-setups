...
case "$(printf %02d ${HINCBOUND})" in
        01) echo untar every hour
            tar -C ${INPDIR} -xvf ${GCM_DATADIR}/year${YYYY}/ERA5_${YYYY}_${MM}.tar
            ;;
        02|03|04|06) echo untar every ${HINCBOUND} hour
            COUNTPP=0
            for (( HOUR=0; HOUR<=23; HOUR=$((HOUR+HINCBOUND)) ))
            do
               tar --wildcards -C ${INPDIR} -xvf ${GCM_DATADIR}/year${YYYY}/ERA5_${YYYY}_${MM}.tar ${YYYY}_${MM}/${GCM_PREFIX}*$(printf %02d ${HOUR}).ncz &
               (( COUNTPP=COUNTPP+1 ))
               if [ ${COUNTPP} -ge ${MAXPP} ]
               then
                 COUNTPP=0
