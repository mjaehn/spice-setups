...
if [ ! -d ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM} ]
then
  mkdir -p ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}
else
  if [ ${CURRENT_DATE:6:4} -eq 0100 ]
  then
    rm -rf ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/*
  else
    # in case of sub-monthly chunks delete only files
    find ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}  -maxdepth 1 -type f -exec rm -f {} \;
  fi
fi
...
# ----------------------------------------------------------------------
cd ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}
...
# num_restart_procs=0
   ...
   if [ -e ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/restart_ATMO_DOM01.nc ];then
    unlink ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/restart_ATMO_DOM01.nc
   fi
   ln -sf ${resdatafile} ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/restart_ATMO_DOM01.nc
  else
# num_restart_procs>0
   RESTART_WRITE_MODE="dedicated procs multifile"
   resdatafile=${RESDIR}/*restart_ATMO_*${RES_DATE}*.mfr
   if [ -e ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/multifile_restart_ATMO.mfr ];then
    unlink ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/multifile_restart_ATMO.mfr
   fi
   ln -sf ${resdatafile} ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/multifile_restart_ATMO.mfr
  fi
...
# Kinne aersols
ln -sf ${KINNE_DIR}/EURO_R13B05_aeropt_kinne_sw_b14_coa.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_kinne_sw_b14_coa.nc
ln -sf ${KINNE_DIR}/EURO_R13B05_aeropt_kinne_lw_b16_coa.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_kinne_lw_b16_coa.nc
#KK ln -sf ${KINNE_DIR}/EURO_R13B05_aeropt_kinne_sw_b14_fin_${YYYY}.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_kinne_sw_b14_fin_${YYYY}.nc
#KK ln -sf ${KINNE_DIR}/EURO_R13B05_aeropt_kinne_sw_b14_fin_${PREV_YYYY}.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_kinne_sw_b14_fin_${PREV_YYYY}.nc
#KK ln -sf ${KINNE_DIR}/EURO_R13B05_aeropt_kinne_sw_b14_fin_${NEXT_YYYY}.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_kinne_sw_b14_fin_${NEXT_YYYY}.nc
# Kinne natural aerosols only
ln -sf ${KINNE_DIR}/EURO_R13B05_aeropt_kinne_sw_b14_fin_1850.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_kinne_sw_b14_fin.nc
ln -sf ${KINNE_DIR}/EURO_R13B05_aeropt_kinne_sw_b14_fin_1850.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_kinne_sw_b14_fin_${PREV_YYYY}.nc
ln -sf ${KINNE_DIR}/EURO_R13B05_aeropt_kinne_sw_b14_fin_1850.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_kinne_sw_b14_fin_${NEXT_YYYY}.nc
# volcanic aerosols
ln -sf ${VOLC_DIR}/bc_aeropt_cmip6_volc_lw_b16_sw_b14_${YYYY}.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_cmip6_volc_lw_b16_sw_b14_${YYYY}.nc
ln -sf ${VOLC_DIR}/bc_aeropt_cmip6_volc_lw_b16_sw_b14_${PREV_YYYY}.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_cmip6_volc_lw_b16_sw_b14_${PREV_YYYY}.nc
ln -sf ${VOLC_DIR}/bc_aeropt_cmip6_volc_lw_b16_sw_b14_${NEXT_YYYY}.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_aeropt_cmip6_volc_lw_b16_sw_b14_${NEXT_YYYY}.nc
# MACv2 simple plumes
#ln -sf ${SP_DIR}/MACv2.0-SP-merged-historical-and-SSP2-45_v1.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/MACv2.0-SP_v1.nc
if [ ${SCEN}=="historical" ] ; then
   SP_filename=MACv2.0-SP-merged-historical-and-SSP2-45_v1.nc 
else
   SP_filename=MACv2.0-SP-merged-historical-and-SSP${SCEN:3:1}-${SCEN:4:2}_v1.nc
fi
ln -sf ${SP_DIR}/${SP_filename} ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/MACv2.0-SP_v1.nc

#Link ozone data:
# new version - for eval and hist, use bc_ozone_historical_YYYY.nc for 2015-2021 (ssp245 were copied for 2015-2021)
#               for ssp??? starting in 2015, use bc_ozone_ssp???_YYYY.nc and bc_ozone_hist_2014.nc in the first year

OZONE_DIR_PREV=$OZONE_DIR
SCEN_PREV=$SCEN

if [ ${SCEN} != "historical" ] ; then
    if [ ${YYYY} -lt 2015 ] ; then
      echo "stop! Year $YYYY is not valid for SCEN=$SCEN"
      exit 
    elif [ ${YYYY} -eq 2015 ] ; then
      OZONE_DIR_PREV=${OZONE_DIR}
      OZONE_DIR=${OZONE_DIR}/SSP_scenarios
      SCEN_PREV=historical # for 2014, no ssp data are present
    else
      OZONE_DIR=${OZONE_DIR}/SSP_scenarios
      OZONE_DIR_PREV=${OZONE_DIR}
      SCEN_PREV=$SCEN
...
ln -sf $OZONE_DIR_PREV/bc_ozone_${SCEN_PREV}_${PREV_YYYY}.nc bc_ozone_${PREV_YYYY}.nc
ln -sf $OZONE_DIR/bc_ozone_${SCEN}_${NEXT_YYYY}.nc bc_ozone_${NEXT_YYYY}.nc
ln -sf $OZONE_DIR/bc_ozone_${SCEN}_${YYYY}.nc bc_ozone_${YYYY}.nc


#Link solar irradiance data
ln -sf ${SOLAR_DIR}/swflux_14band_cmip6_1850-2299-v3.2.nc ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/bc_solar_irradiance_sw_b14.nc
...
echo "NDYN_SUBSTEPS=${NDYN_SUBSTEPS}" > ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/dt_settings
echo "DTIME=${DTIME}" >> ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/dt_settings
echo "DT_CONV=${DT_CONV}" >> ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/dt_settings
echo "DT_RAD=${DT_RAD}" >> ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/dt_settings
echo "DT_SSO=${DT_SSO}" >> ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/dt_settings
echo "DT_GWD=${DT_GWD}" >> ${WORKDIR}/${EXPID}/joboutputs/icon_make_FG/${YYYY_MM}/dt_settings
...
/
&output_nml
!-----------------------------------------------------------Output Namelist 15: dwd_fg-file
  filetype                     =  4              ! output format: 2=GRIB2, 4=NETCDFv2
  dom                          =  1              ! write all domains
!  output_bounds                =  ${SSTART}, ${SNEXT}, ${SOUT_INC[$((NOUTDIR+=1))]}  ! start, end, increment
  output_start                 = "${YDATE_STOP_ISO}" 
  output_end		       = "${YDATE_STOP_ISO}"
  ...
  output_filename              = '${INIDIR}/FG_HIST'
  filename_format              = '<output_filename>_<datetime2>'   ! file name base
  ml_varlist                   = 'group:mode_dwd_fg_in', 'group:dwd_fg_atm_vars', 'group:dwd_fg_sfc_vars'
  output_grid                  =  .TRUE.
