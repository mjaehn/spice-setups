...
EOF_MESS
  ...
  echo check ${WORKDIR}/${EXPID}/joblogs/icon/icon_${EXPID}_${YYYY_MM}_check_files_${JOBID}.log >> ${PFDIR}/${EXPID}/error_message
...
then
  ...
  if [ ${SCEN} == "historical" ] ; then
    ...
    fg_file=${HIST_INIDIR}/FG_HIST_20150101T000000Z.nc
  fi
...
ln -sf ${SP_DIR}/${SP_filename} ${WORKDIR}/${EXPID}/joboutputs/icon/${YYYY_MM}/MACv2.0-SP_v1.nc
...
# set some namelist switches for climate projections
lscale_cdnc=.false.
irad_aero=18

if [[ ${YYYY} -ge 2015 && ${SCEN} != "historical" ]]; then
  lscale_cdnc=.true.
  irad_aero=19
fi
...
/
&initicon_nml
  init_mode                    = 1       !for the initialization of the atmosphere caf1950010100_ini.nc , first guess file (soil data) IAEVAL00 
...
!-----------------------------------------------------------Output Namelist 14: c-file
  ...
  ml_varlist                   = 'z_ifc','z_mc','topography_c','fr_land','depth_lk','fr_lake','soiltyp','fr_urb_smt','rootdp','fr_glac' !'fr_urb','fr_glac',fr_glac might cause problem
  output_grid                  =  .TRUE.
!  stream_partitions_ml   =  2
/
